...
**Version:** 4.4 (ALL phases 0–16 + 9b v2 console done; v0.21 edge-case hardening; v0.21.1–0.21.38 full history in §17; **v0.21.39 pasted-JSON-spec direct-execute** (the JSON spec IS the contract — never re-interpreted; verified live on the owner's server: 259-op fortress staged exact); **v0.21.40 📎 upload button + vision** (`.json` upload → staged directly without pasting 140KB specs; image upload → vision provider → JSON build spec → staged); smoke **355/355**, jar `GHBot-0.21.40.jar`)

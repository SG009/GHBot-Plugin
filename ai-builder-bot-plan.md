...
4. **Sandbox constraints (critical):** toolchain resets every turn → run `bash gh-bot/tools/setup-build.sh clean build`; workspace snapshot excludes `build` dirs (`dev.ghbot.builder`, never `dev.ghbot.build`) and `/tmp`; Gson blocked → hand-rolled JSON (`JsonUtil`, `JsonBuildSpec`); Paper CAN boot in sandbox (2 GB box: `-Xmx640M -XX:+UseSerialGC -XX:MaxMetaspaceSize=192M -XX:MaxDirectMemorySize=64M -Xss512k`, flat world, distances 2, jar via fill.papermc.io v3 — api v2 sunset) → verification = headless smoke (`tools/SmokeTest.java`, currently 620 checks) + sandbox-local live runs + owner's live logs. Ship to `/home/user/releases/GHBot-<ver>.jar`, bump `build.gradle.kts` version, and ALWAYS delete the previous version's jar in the same commit (owner policy — releases/ holds only the newest). Same policy on the GitHub Releases page: delete the previous release + tag, create `v<ver>` with the jar attached (`gh-bot/tools/github-release.sh`).
...
- **v0.22.0 — JARVIS-FOR-ADMIN (4-PILLAR RESET) — ✅** (owner's decisive refocus: "GHBot going back to many ingredients but cooking nothing… delete all unrelated features… make it a Jarvis-like for the server that only admin can use"):
  - **Surface cut to 4 pillars** in `BotCommands.CATALOG` (single source for help/tools/auto-detect): **1 Build** (`build/plan/edit/schem(+import)/paste/library/export/approve/deny/redo/cancel/view`), **2 Passive eyes + edit** (`scan/find/look/set/replace/terraform/undo`), **3 Manage** (`status/cap/device-info/provider/refresh/confirm/admin/cmd`), **4 Interact** (NL in chat-console, `@GH000 <cmd>` in-game, `@GH000 chat` for the Technician).
  - **Shelved** (removed from surface, blocked at dispatch with a clear message, code kept in git): `avatar, marker, workers, deploy, undeploy, where, save-location, list-locations, delete-location, teach, dataset, critique, design, image, memory, debuglog, animate, add, editspec, schem download`. `BotCommands.SHELVED` set + `CommandBridge.dispatch` guard.
  - **Admin-only:** dispatch hard-blocks non-OP players (`GH-bot is admin-only — console/OP required.`); console + ops only (ChatListener op-gate already existed).
  - AutoTools / ToolProtocol help / CapabilityGuide / secretary KEY_COMMANDS all trimmed to the surface.
  - Smoke **384/384 PASS** (new: catalog-surface, shelved-blocked, admin surface size). `GHBot-0.22.0.jar`.
  - **Next (already scoped):** Pillar 2 → `scan/find/look` emit jsonspec (world-as-data for smarter edit/set/replace/terraform/undo); Pillar 3 → `cmd` output captured from the server console into the chat-console (Logger-handler research). Owner offered to research: Paper console-output capture, Litematica bit-packing edge cases, FAWE/WorldEdit fast paste on Paper.
- **v0.22.1 — EYES-AS-DATA (PILLAR 2 jsonspec) + HARDENING — ✅** (owner decisions: surface-only scan default + `--full [depth]`, inline budget 150 blocks, memory+`logs/eyes/`+inline routing, freeze-not-delete shelved code, enforce vanilla block constraints):
  - **`TerrainSpec` (new, `dev.ghbot.terrain`):** the same contract shape as a JSON build spec (`{name,palette,blocks[]}`) but in **absolute world coords + an `origin` anchor**. `scan` (surface snapshot by default; `--full [depth]` adds bounded depth, cap 64) / `find` (found coords) / `look` (single block with full blockstate preserved) all emit one. Routing: bot memory `eyes.spec` + `logs/eyes/<name>.json` (full data) + a **bounded 150-block inline JSON** to the chat/AI context (`TerrainSpec.INLINE_MAX`). `TerrainSpec.toBuildSpec()` translates absolute→relative via the origin and round-trips into the existing build/edit path (see→act loop).
  - **Vanilla block constraint:** `JsonBuildSpec` now resolves every block through `Material.matchMaterial` and reports *why* a spec is invalid (was: silently skip hallucinated names at placement → "fewer blocks than spec" drift). Added `JsonBuildSpec.stripState()` for uniform `[state]` handling.
  - **Heightmap persistence:** `TerrainSummary.toMap()`/`fromMap()` now save/restore the heightmap (was dropped on reload); downsampled for huge scans so the phone's session YAML stays small.
  - **Coordinate parsing unified:** `CoordResolver.scanTarget()` + `parseWhere()` — one parser shared by `scan/find/look/set/replace/terraform` **and** `AutoTools`. Fixes the v0.21.44 batch-test gap: `scan 100 at 86 86 262` / `scan 86 86 262` / `scan at 86,86,262` all resolve; `set stone at 86 86 262` accepted. `find that/this` pronoun fallback → last scan's dominant top block (no more brittle "unknown block").
  - **Admin-only guard extracted** to `CommandBridge.isAdminSender()` (documented + smoke-testable). **`ToolBridge.LEGACY_ALLOWED` removed** — `ALLOWED == CATALOG.keySet()` is now the single source of truth. **Shelved freeze policy** documented in `BotCommands.SHELVED` + `// SHELVED v0.22.0` markers on `LocationCommands`/`AvatarCommands`.
  - **Sandbox build story:** added `gh-bot/tools/sandbox-build.sh` + `make-jar.py` — reproduce compile+smoke+jar when only GitHub/PyPI/npm egress exists (PyPI `jdk4py` JRE 21 + Eclipse ECJ 3.37 compiler + a public repo's committed paper-api 1.21.11 dependency cache). Gradle `setup-build.sh` unchanged for normal use.
  - Smoke **414/414 PASS** (+30 checks: eyes round-trip, inline truncation, vanilla constraints, heightmap persistence, coordinate parsing, pronoun fallback, shelved surface, admin-only). `GHBot-0.22.1.jar` in `releases/`.
  - **Batch-test fixes (owner's 2026-08-20 evidence — note the owner tested v0.22.0, so eyes-as-data still needs a live re-test on 0.22.1):**
    - **AI prompt advertised shelved tools** — `ChatService.SYSTEM` still listed `where, list-locations, save-location, workers, deploy, undeploy, marker, avatar, critique` and `CapabilityGuide` listed `schem download` + `cmd gh save-location`; the model consequently hallucinated `schem download`/`save-location`. Both trimmed to the 4-pillar surface; new smoke check asserts no unambiguous shelved name leaks into either prompt.
    - **`edit the dragon's wings`** → `Couldn't resolve "the"` (edit takes args[0] as target). Now leading determiners (`the/a/an/that/…`) fall back to `here` and keep the phrase as the instruction.
    - **`paste <file> 30 10` ignored the offset** (pasted at sender base/origin). Now parses `x z` / `x y z` / `at …` / player offsets via a new `CoordResolver.offset()` helper.
    - Guardrails verified live: `cmd stop` → BLOCKED `CONF-37514663-568` → `confirm` → ran (server stopped cleanly); `deny` cleared a 386,520-block citadel ghost cleanly; admin-only held. Env quirks re-confirmed: Ollama 429 session-usage-limit + `request timed out` on `plan` (Stop button is the escape hatch).
  - Smoke **420/420 PASS**. `GHBot-0.22.1.jar` re-rolled in `releases/` (owner never ran the first 0.22.1 build).
  - **Next open work:** Pillar 3 `cmd` output capture (Logger-handler), render eyes-specs in the 3D viewer, vision quality auto-verify loop, `.mcstructure` export, FAWE fast-paste, viewer local-mode, `catalog` auto-refresh on empty (AI had to run `refresh` manually), Litematica import (paste of `.litematic` still unsupported).
- **v0.22.2 — PILLAR 3 CMD OUTPUT CAPTURE + PILLAR-1/2 AUDIT FIXES — ✅** (owner flow: deep-research Pillar 3 first, then audit P1/P2 for go/no-go, then GO full scope; D2 console-tail later, D4 no async settle):
  - **Pillar 3 — every `cmd` path now shows the server's real reply.** Root cause found by deep research (`gh-bot/docs/PLAN-pillar3-cmd-output-capture.md`): on Paper 1.21, `CommandSender` extends Adventure `Audience` and *every* Component message funnels into a terminal default whose body is empty — the old String-only `ToolBridge.Capture` silently dropped all Component output (LuckPerms/DeluxeMenus/vanilla feedback), producing the misleading \"output went to the server console\" note. Fix ships as `CapturingSender` (legacy String + Adventure terminal + bungee `spigot()` — the old `null` spigot was a latent NPE) + a session-scoped **JUL root handler** for plugins that log instead of replying. **Log4j2-appender alternative was dependency-blocked**: log4j-core is absent from paper-api's compile closure, from the ZoneGuard fallback cache, and from Apache's GitHub release assets (all verified; evidence in PLAN doc). JUL chosen: JDK-only, smoke-testable, covers the dominant case (plugins log via JUL).
  - **All four paths rewired** through `CmdOutputCapture`: AI tool/`dispatchCaptured` (contract kept), web `/cmd`, in-game `cmd`, and `confirm <CONF-token>` (D5 — confirmed commands capture too). Eyes-style routing: bounded inline (web 4000c/40L, in-game 1500c/15L) + full output → `logs/cmd/<file>.log` when truncated (WIBLogger.writeCmdOutput, mirrors writeEyesSpec).
  - **Pillar-1/2 go/no-go audit** (`gh-bot/docs/AUDIT-pillar1-2-go-no-go.md`, verdict GO with fixes):
    - **P1-1 (HIGH) template-edit bbox Y/Z transposition** — `templateEditSpec` read `bbox()`'s `{minX,minY,minZ,maxX,maxY,maxZ}` as `{minX,minZ,minY,maxX,maxZ,maxY}`; every no-AI-fallback roof/column/door/window/tree anchor wrong when height≠depth. Survived because smoke fixtures were Y/Z-symmetric (10×10×10 cube = identity under the swap). Fixed + asymmetric fixtures added; **mutation-validated against the shipped 0.22.1 jar** (old: roof y=7, cz=2, col top 9 — exactly the audit's prediction; fixed: 6, 3, 8).
    - **P1-2 Litematica negative `Size`** — real-world regions signed → 0-block decodes; `abs()` fix + synthetic-NBT smoke. Backlog note corrected: `.litematic` import exists since v0.21.46 (the \"unsupported\" line was stale).
    - **P1-3 paste/import confinement** — `resolveInLibrary()` rejects `../` (both SchematicCommands `paste` and DatasetCommands `schem import`).
    - **P1-4 `/upload` bounded read** — 25 MB cap enforced DURING the read, not after full buffering.
    - Doc truths: terraform = `flatten` only (smooth/raise/lower never implemented — COMMANDS.md fixed); undo restores types only.
  - **Doc-drift guard added earlier in this work:** `tools/check-docs.sh` + a CI step assert README/version/smoke-count claims match the actual suite output (the 414-vs-420 drift class is now CI-fatal).
  - Smoke **454/454 PASS** (+34: all-surface capture incl. the previously-dropped Component terminal, CmdOutput merge/inline/budgets, JUL format, wiring + blocked contract + headless degradation, confinement, asymmetric bbox, negative-size Litematica). `GHBot-0.22.2.jar` in `releases/`.
  - **Next open work:** owner live test of Pillar 3 (acceptance checklist in PLAN doc §8 — esp. `cmd lp listgroups` for the Component path); Q1 undo drift-guard; Q2 terraform modes; Q3 web-console auth token; eyes-specs in 3D viewer; console-tail feed (needs the log4j-core dependency question); vision auto-verify; `.mcstructure` export.
- **v0.22.3 — LIVE-BATCH REGRESSION SWEEP: cmd dispatch fixed on Paper 1.21 + CONF loop + admin read + scan y<0 — ✅** (trigger: owner's 0.22.2 batch evidence — every cmd "✗ failed", 5-token confirm loop, admin read "Path escapes server dir.", scan 0/537920; full story in `gh-bot/docs/FIX-0.22.3-cmd-dispatch.md`):
  - **R1 — every `cmd` failed on Paper 1.21.** Root cause (paper-source verified + probe-proven): `CraftServer.dispatchCommand` runs `VanillaCommandWrapper.getListener(sender)` on EVERY dispatch, which **throws** `IllegalArgumentException` for plain custom senders — and v0.22.2's single try/catch let that throw also skip the console fallback (0.22.1's two-try shape was why commands used to "run"). Fix: dispatch through Paper's own `io.papermc.paper.commands.FeedbackForwardingSender` (its `asVanilla()` is the accepted hook; all feedback — legacy String, Adventure, vanilla `sendSystemMessage` — funnels into our consumer, OWNER vanilla permission). Class lives in paper-**server** → referenced by **reflection only** (new `FeedbackForwarder`, zero new deps; Spigot/old Paper/headless → console fallback, one-time info note). Dispatch throws are no longer swallowed — FAILED carries a reason note (`✗ failed: stip — unknown to the server (try refresh / catalog <kw>)`) rendered via CmdOutput.fix(note-dropped-by-file-routing).
  - **R2 — CONFIRM loop.** v0.22.2's confirm re-ran the systemic guard → re-BLOCKED + re-minted forever (`confirmed but command failed`). Now `capture(bot, line, bypassGuard=true)` is used only by confirm — the CONF-… token IS the confirmation. Live: `whitelist off` → confirm → **actually ran**; `stop` → confirm → **server halted**.
  - **R3 — admin read "Path escapes server dir."** `detectServerRoot()` can return relative/empty (`File(".")` world container) → absolute `resolveFile` never startsWith a relative root → every whitelisted server file rejected. Fixed by `toAbsoluteRoot()` at construction (probe-validated live: `worldContainer=[.]`, old threw, new reads).
  - **R4 — scan blind below y=0.** `Math.max(minHeight, 0)` pre-1.18 clamp; now `columnMinY(w.getMinHeight())`. Live flat-world contrast: OLD minY=0 → 0 non-air; NEW minY=-64 → **6724/645504**.
  - **Audit single-point + truth:** dispatch audit moved into `CmdOutputCapture.capture()` (covers AI/web/in-game/confirm — previously AI-only, and BLOCKED was logged as "ok"). New doc rule: **never claim without a live-or-smoke check** — this sweep was validated by a sandbox-local Paper 1.21.11-132 (exact owner build, via fill.papermc.io v3 — api v2 is sunset) driving the real web `/cmd`.
  - Smoke **471/471 PASS** (+17: FF route feature-detect, confirm-no-re-mint + one-shot, audit-once, note preservation, root anchoring, column min-Y). `GHBot-0.22.3.jar` in `releases/` (0.22.2 deleted per policy).
  - **Known quirk:** `/gh <botcommand>` via web `/cmd` still executes + audits, but its reply text races the HTTP response (registry subs run on the bridge async pool by design) — chat/in-game surfaces unaffected.
  - **Next open work:** owner re-run of the SAME batch; Q1 undo drift-guard; Q2 terraform modes; Q3 web-console auth token; eyes-specs in 3D viewer; console-tail feed; vision auto-verify; `.mcstructure` export.
- **v0.22.4 — JAR VERSION-STAMP FIX (`version GHBot` reports the true build) — ✅** (trigger: owner's 0.22.3 batch was ALL-GREEN but `TOOL cmd version GHBot` replied `GHBot version 0.22.2`; full story in `gh-bot/docs/FIX-0.22.4-version-stamp.md`):
  - **Root cause (reproduced in-lab, Gradle 8.10.2):** `processResources` `expand(...)` properties are NOT up-to-date inputs — after the 0.22.2→0.22.3 bump without `clean`, the jar task (named by the NEW version) re-assembled cached OLD resources: `GHBot-0.22.3.jar` contained `version: '0.22.2'` alongside brand-new v0.22.3 classes. In-lab: jar named 0.22.5-TEST carried a 0.22.4 stamp until the fix; identical failure shape as the shipped jar.
  - **Fix:** `inputs.property("version", project.version)` in `processResources` → stamp re-renders on every bump (proven: same no-clean build flips to the new stamp).
  - **Guards:** smoke pins (classpath `plugin.yml` stamp must equal `build.gradle.kts` `version`; mutation-validated to FAIL on drift) + a `check-docs.sh` jar-stamp check at ship time.
  - **Live-validated** on sandbox-local Paper 1.21.11-132 @c5eb079 (= owner build): banner `Loading server plugin GHBot v0.22.4`, web `/cmd version GHBot` → `GHBot version 0.22.4`, `plugins` intact.
  - Smoke **473/473 PASS** (+2). `GHBot-0.22.4.jar` in `releases/` (0.22.3 deleted per policy); GitHub Release v0.22.4 created (v0.22.3 release + tag deleted per mirror policy).
  - **Notable:** owner's batch ALL-GREEN on 0.22.3 — chat `admin set motd` edited `server.properties` live (with backup + rollback token), confirm ran `stop` first try, scan y<0 found the ground. GHBot now edits any whitelisted server file end-to-end.
  - **Next open work:** Q1 undo drift-guard; Q2 terraform modes; Q3 web-console auth token; eyes-specs in 3D viewer; console-tail feed; vision auto-verify; `.mcstructure` export; backlog unchanged.
- **v0.23.0 — WEB-CONSOLE LOGIN TOKEN (Q3; batch Phase A) — ✅** (owner design: "CONF-style token shown at server console — only real-admin sees it"; batch plan `gh-bot/docs/PLAN-next-batch-v0.23-v0.27.md`):
  - **Gate:** every web route (`/`, `/chat`, `/console`, `/cmd`, `/upload`, `/api/*`, `/view…`) requires a session cookie; unauthenticated → 302 `/login` (pages) or 401 JSON (`/api/*`, `/cmd`, `/upload`). Only `/login` + `/api/login` are public. New `WebAuthService` (pure logic, headless-pinned) + a request-time `guard()` wrapper in `WebStatusServer` (contexts register before auth attaches).
  - **Token:** `WEB-%08d` minted with **SecureRandom** (unlike time-based CONF mint — auth needs unpredictability), printed ONCE per boot in the server console / latest.log; fresh mint every restart. Optional fixed token `server.web.token` (config.yml resource documents it; regen refused in fixed mode). Constant-time compare (`MessageDigest.isEqual`), token never logged outside the startup/regen lines.
  - **Sessions:** HttpOnly cookie, 128-bit id, 12 h sliding TTL, in-memory (restart = logout). **Brute-force:** 5 fails/min per IP → 10 min lockout; correct-token-while-locked still rejected; fails/lockouts/auth-blocks audit-log to the GHBot web log (ip + count, never the credential).
  - **`webtoken` builtin** (op-only via the v0.22.0 admin rule): regen → mints new token, kills all sessions, replies to the op AND logs the new token to the server console (console-log-rescue — live testing caught that a web-only owner would otherwise lock themselves out, since in-game replies race the web `/cmd` HTTP response).
  - **Live matrix on sandbox-local Paper 1.21.11-132 (= owner build):** no-cookie → 302/401; login page the only public route; wrong×5 → 429 (locked 600 s); boot mint differs across restarts; cookie passes `/cmd plugins` + `/view`; old boot token 401; regen kills cookie + old token; regen token recoverable from latest.log (rescue path verified end-to-end).
  - Smoke **493/493 PASS** (+20: 15 auth-rule pins — mint format, regen uniqueness, constant-time semantics, cookie round-trip + forgery rejection, per-IP lockout, locked-while-correct, TTL expiry via injected clock, regen session-kill, fixed-token mode, config parse — + 5 real-HTTP wire pins against a headless HttpServer). `GHBot-0.23.0.jar` in `releases/` (0.22.4 deleted per policy); GitHub Release mirrors.
- **v0.23.1 — LOGIN UX: RETURN-TO-DESTINATION — ✅** (owner report: login dumped everyone on the status page '/' instead of the chat console):
  - Unauthenticated browsers now bounce to `/login?next=<original path>` and land back there after login; default landing **`/console`** (owner's main tool); status page links the console.
  - `sanitizeNext()` open-redirect guard (same-site absolute paths only; `//evil.com`, scheme/control/quote tricks → `/console`).
  - Smoke **500/500 PASS** (+7: sanitize rules, next-carry, console default, status link). Live curl matrix 7/7 on the owner build (bounce→login→`/console` 200 real page; evil next blocked). `GHBot-0.23.1.jar` shipped (0.23.0 deleted per policy; GitHub Release mirrored).
- **v0.24.0 — CONSOLE-LOG AUDITOR (owner batch Phase B) — ✅** (owner scope: "GHBot will only tell if there's error, warning and updates of any plugin or even the papermc… tells what those are + suggests. Don't make the console agent also turn into server console. Why we need 2 server logs right?"):
  - **`LogWatch`** — reflection-only dynamic-Proxy log4j2 appender on the ROOT logger config (zero compile deps; off + one-line note when log4j-core absent): bounded ring 200, consecutive-repeat collapse, IPv4 strip `<ip>`, bounded 8-frame stacks, clean `onDisable` detach. **Design cross-validated against DiscordSRV/JDAAppender** (owner-suggested deep research): same root attach + level gating + `isStarted=true`; GHBot additionally keeps FATAL (their level map drops it) and the ring is bounded (their queue isn't).
  - **`AuditService`** — severity-first grouped digest (≤6 groups + radar line): attribution logger-prefix → stack-frame → server-core → **thread-name fallback** (live catch: Paper's update banner logs with NO logger); suggestion rules scan the **whole group incl. stacks** (class/java-version/enable/deprecated/OOM/net/TLS/exception/release-behind/AI-401); mixed-line groups render truthfully (`×6, 5 lines` + most-informative line; never a fake `×N`); honest age (`since boot` without a stamp).
  - **`UpdateRadar`** — installed-only async checks (first pass +60 s, then daily), quiet-once notify persisted to `updates-notified.yml`: Paper via fill v3, Essentials via GitHub, Geyser/floodgate/Via*/LuckPerms via Modrinth v2 (loader-filter pick — geyser's first entry is `-Velocity`); SnakeYAML-as-JSON parser (already on classpath).
  - **`audit [updates|clear|selftest]`** bot command (all surfaces incl. AUTO-TOOL "any errors?"/"check for updates"); `selftest` emits a real WARN to prove the listener end-to-end.
  - **Live run (sandbox = owner build 1.21.11-132):** attached silently; digest live: `• server (WARN ×6, 5 lines): "However, you are 4 release(s) behind the latest stable release (26.2)!" — server reports a newer release — schedule an update…` · `• GHBot (WARN ×5, 3 lines): "…HTTP 401…" — the AI provider rejected the key — set a valid api-key under ai: …` · `updates: all current (1 checked)` (132 == fill latest). selftest ×3 collapse ✓; AUTO-TOOL ✓; v0.23.1 auth matrix re-green; `/cmd`-meets-`/gh` async-by-design doc holds (use chat surface for bot commands).
  - **Live-polish inside the release (3 catches from the live run):** nameless-logger "?" → `fromThread` (PaperMC/Server/netty-io) + core mapping; multi-line banner lied `"***" ×6` → distinct-truthful render + representative heuristic; suggestion scanned only the newest line → whole-group hay (+ provider-401 rule, seen live: polls now requires a key). Three mutation suites killed exactly these pins.
  - Smoke **533/533 PASS** (+33). `GHBot-0.24.0.jar` shipped (0.23.1 deleted per policy; GitHub Release mirrors).
  - **Next open work (batch plan):** Phase C eyes + good-result build pack · Phase D terraform brushes · Q1 undo drift-guard · vision auto-verify · `.mcstructure` export.

- **v0.25.0 — EYES LATTICE + GOOD-RESULT BUILD PACK (owner batch Phase C) — ✅** (owner direction: "im still confuse… do deep research for me and tell me your ideas!" — research said: don't fine-tune (hosting is the unaffordable part), do retrieval-augmented few-shot + hand-editable style sheets + two-pass generation; skeleton pre-pass deferred as the phone-unsafe 20%):
  - **Eyes lattice:** `TerrainSummary.topMaterials` + stride tiers (≤8→1, 9–32→2, >32→counts-only) → per-column `x,z: y material` absolute grid, budget-trimmed truthfully; full grid → `logs/scan/grid-<ts>.log`. Viewer `scan` checkbox renders `/api/scan/last` with the SAME block renderer (voxels re-based; backup/restore of the build preview).
  - **One composer, every surface:** `TerrainCommands.toolScanReply` serves registry `scan` AND the AI/AutoTool path — the live-caught pre-ship drift (tool path answered counts-only → chat empty, API 404) is pinned by a smoke drift-guard reading `GHBotPlugin.java`.
  - **Look-then-set precision:** strict AUTO-TOOL `set|place <block> at x y z` (negative coords; truthful unknown-block), CATALOG usage fixed, prompt block teaches scan→math→`set`, never tell the admin `/setblock`. Live-verified in-world via vanilla `execute if block` + `undo`.
  - **Good-Result pack:** `teach <name> [staged] gold` → palette-compressed verbatim exemplar (≤80 blocks, truthful skip note) injected into `buildReferencePrompt` alongside compactLines; `plugins/GHBot/styles.yml` (abandoned/medieval/modern/rustic — decay %, vines, no-symmetry hints) injects on prompt-word match; two-pass generation plan→parts→per-part validate+ONE feedback retry→merge with honest `(partial: N part(s) dropped)` name. teach/dataset un-shelved per freeze-policy procedure (contract pins updated).
  - **Live run (sandbox 1.21.11-132):** lattice in chat reply + grid file ✓ · `/api/scan/last` 81 cols @ r=4 ✓ · set→world→undo truth-cycle ✓ · NL "place gold_block at -2 81 -2" → placed ✓ ("gold" → truthful reject) ✓ · styles.yml boot copy "style sheets loaded: 4" ✓ · audit/auth regressions green ✓ · clean shutdown ✓.
  - Smoke **555/555 PASS** (+22; mutation suites A/B/C each killed exactly their pins pre-ship). `GHBot-0.25.0.jar` shipped (0.24.0 deleted per policy; GitHub Release mirrors).
  - **Next open work, pre-v0.26.0 note:** Phase D terraform brushes (raise/lower/smooth + dry-run) · Q1 undo drift-guard · vision auto-verify · `.mcstructure` export + FAWE research · owner-proposed **audit fix-advisor** (browse/drill-down + knowledge-base fix advice — awaiting green light).

- **v0.26.0 — AUDIT FIX-ADVISOR (owner-proposed: "hand me the admin advice on how/what it needs, correctly") — ✅**:
  - **Numbered digest groups + browse footer** (`audit show 1..N`): `audit show <n>` renders every captured line + stack of group #n (bounded 25 lines, truthful trim note, sane-age floor so synthetic stamps never claim absurd minutes). ONE shared `orderedGroups` ordering feeds digest/show/fix (group numbers can't drift between surfaces).
  - **`audit fix <n>`** answers from **plugins/GHBot/audit-fixes.yml** (SnakeYAML, hand-editable, `audit reload` live): owner rules match BEFORE the 19 built-ins (release-behind → world-corruption), `%s` = source name, provenance always shown (`audit-fixes.yml`/`built-in`). No match → ONE clearly-labeled "🤖 AI guess (unverified)" via a dedicated chat memory lane (`ChatService.askOnce` — never pollutes the admin's real session); no provider → honest "add a rule" (no fake advice, no silence).
  - **Owner-evidence honesty fixes:** `audit updates` answers the CURRENT table synchronously (plus truthful "last checked N min ago" + silent background re-check) — the v0.24.0 "results arrive in a moment" that stayed quiet is gone; `⚠ pre-release` risk tags on SNAPSHOT targets (their ollama had said "all three can be upgraded safely").
  - **Live catches shipped inside the release:** literal `audit reload|updates|clear|selftest` fell through to the AI (never ran provider-less) → deterministic AUTO-TOOL passthrough + pin; jar-stale teach during smoke (rebuild discipline). Live sandbox run: file-rule precedence (edited yml + `audit reload` + next answer carried the edit), show-all-lines browse, selftest rule, sync table, out-of-range truths.
  - Smoke **579/579 PASS** (+24); mutations D/E/F killed exactly their pins. `GHBot-0.26.0.jar` shipped (0.25.0 deleted per policy; GitHub Release mirrors).
  - **Next open work (batch plan):** Phase D terraform brushes (raise/lower/smooth + dry-run) · Q1 undo drift-guard · vision auto-verify · `.mcstructure` export + FAWE research · viewer local-mode · `schem download` revival.

- **v0.27.0 — PHASE E START (Phase D ❌ dropped by owner: "skip the phase D and proceed the phase E, i dont think GHBot should have that feature anyway") — ✅**:
  - **Q1 undo drift-guard:** undo verifies natural drift before restore — exact per-position compare (`Change.newType` vs live world; unreadable ⇏ drift), non-destructive refusal via new `UndoManager.peekForUndo` (stack survives), `undo confirm [minutes]` force path, both outcomes audit-logged to `logs/edits.log`. Live: set → vanilla-setblock drift → refuse (drifted block SURVIVES) → confirm → reverted with truthful "(drift overwritten as confirmed)" note.
  - **Small batch:** web `server.web.local-bypass` (viewer local-mode — loopback-only login bypass, secure default OFF, LAN never bypasses; live: unauth /,/console,/api/status → 200 from 127.0.0.1) · catalog auto-refresh on empty at boot+2s (live: 213 commands) · paste-ambiguity prompt (asks with real candidates; live chat evidence).
  - Smoke **589/589 PASS** (+10); mutations G/H/I killed exactly their pins; doc-consistent. `GHBot-0.27.0.jar` shipped (0.26.0 deleted per policy; GitHub Release mirrors).
  - **Next open work (Phase E queue continues, same green light):** vision auto-verify loop (post-build check → self-repair) · `.mcstructure` export + FAWE fast-paste API research · then batch complete → final docs sweep.

- **v0.27.1 — PHASE E item 3: Bedrock `.mcstructure` export + FAWE research — ✅**:
  - New `McstructureCodec` + `LeNbtWriter` (little-endian uncompressed NBT). Spec-accurate: `format_version=1`, `size`/`structure_world_origin` as TAG_List of 3 ints (NOT Int_Array — Bedrock refuses those), two `block_indices` layers in **ZYX** (`i = SZ·SY·X + SZ·Y + Z`), empty cells `-1` (structure void), palette `{name, states{}, version=18168865}`. Java→Bedrock remaps for the persistent name divergences (`grass_block→grass`, `cobweb→web`, …); reverse-map on import so `paste` restages on Java.
  - `export <name> mcstructure` (alias `bedrock`) / `all` now writes `.mcstructure` next to the 5 Java formats. Library lists it. Round-trip import via `SchematicImporter` (LE sniff).
  - **FAWE: research only, no code.** Owner's 6 GB phone already runs Paper+Geyser+Via*+DiscordSRV; FAWE would bypass GHBot undo/ghost/drift-guard; we already export Sponge `.schem` which FAWE eats if the owner later runs a dedicated creative box. Write-up: `gh-bot/docs/RESEARCH-mcstructure-fawe.md`.
  - Smoke **607/607 PASS** (+18); mutations G (XYZ index) / H (size as Int_Array) / I (skip grass remap) killed exactly their pins. Live on Paper 1.21.11-132: staged 3-block JSON spec → `export mcs_live mcstructure` → 409-byte LE file (`0a 00 00 03 0e 00 format_version 01 00 00 00`), namespaced palette, TAG_List size.
  - `GHBot-0.27.1.jar` shipped (0.27.0 deleted per policy; GitHub Release mirrors).
  - **Next open work:** vision auto-verify loop (Phase E item 2) · then batch complete → final docs sweep.

- **v0.28.0 — command-script upload (📎 .txt / .cmd / .mcfunction) — ✅**:
  - New `CommandScript` class: parses uploaded text files into `Line` records (number, step, section, command, sensitive flag, placeholders list). Comments stripped, order kept, leading `/` stripped, trailing `# comment` dropped, BOM tolerated. Section titles parsed from `# N) Title` lines. `MAX_BYTES = 64 KB`, `MAX_COMMANDS = 80` (truthful `truncated` count when over).
  - **Parser is the contract** — same idea as a pasted JSON build spec. Placeholders: `YOURNAME` (case-insensitive), `<player>`/`<name>`/`<username>`/`<foo>`, `{player}`/`{name}`/`{foo}`, `TODO`/`CHANGEME`/`PLACEHOLDER`. `run` **refuses** while any remain; admin fills via chat ("my name is .SerthGembel009") or skips.
  - **Fill / skip / drop via chat:** `parsePlayerName` catches `my name is`, `i'm`, `ign`, `player:`, `username:`, `replace YOURNAME with`, plus the bare Bedrock-style `.Name`. `parseSkipSteps` catches `skip step 6` / `skip 6`. `parseSkipKeywords` catches `skip the boss` / `skip starter-weapon` (filtered against common prose). All three are AUTO-TOOL detected through `AutoTools.detectScript` which runs BEFORE `AutoTools.detect` but only matches when a script is pending — scan/build/cmd/etc never get stolen.
  - **CONF guards still apply** — `CommandLearning.isSensitive` flags each parsed line at parse-time and again after fill; systemic commands (`op`, `stop`, `reload`, `whitelist`, `rm -rf`, …) mint CONF tokens at run-time, same as typing `cmd op Steve` by hand. Nothing is bypassed.
  - **Dispatches through `cmd`'s per-line capture** — `CommandScript.execute` calls `capture.captureMany` (or `dispatchGuardedMany` fallback), so you get real command output + full text in `logs/cmd/<file>.log` for long replies. No separate code path.
  - **The AI is told not to cmd the file itself** — CapabilityGuide + ToolProtocol both say "📎 .txt command scripts are PREVIEWED first (never auto-run)… Do NOT dump the file through cmd yourself — wait for the admin to say run". ChatService SYSTEM prompt also carries "PREVIEWED first (never auto-run)". Pinned in smoke.
  - **`script` command added to CATALOG** (and therefore automatically to `ToolBridge.ALLOWED` since that's just `CATALOG.keySet()`). `run|status|drop|preview|execute` subcommands — the `amend` sub is only used internally when a chat follow-up arrives while a script is pending.
  - **Upload route split:** `handleUpload` now branches on `CommandScript.isScriptFilename(name)` before the JSON/image paths. New `handleScriptUpload` stashes the parsed file into `CommandScript.PENDING` (per bot), injects a note into chat history so the Technician sees "a script was uploaded, don't cmd it yourself", then returns the preview card. **Never executes.**
  - **console.html:** 📎 button `accept` attribute now includes `.txt,.cmd,.mcfunction,text/plain`. `uploadFile` detects script files, reads the input-box text as the admin prompt, sends it as `?prompt=…`, and shows the user's prompt in the upload bubble. Welcome message updated to mention `.txt` command lists.
  - **Smoke 642/642** (+22 over v0.27.2): parse of `setup_commands.txt` (27 commands, 3 YOURNAME holes, step 6 boss, leading-slash-strip, trailing-comment-drop, empty/comments-only/json-as-txt errors, 80-command cap with truthful truncated count) · plan: unfilled holes NOT in toRun · fill YOURNAME + op still CONF · skip step 6 drops boss+spark · skip the boss matches section · preview card has purpose+holes+say-run+not-run-yet · handle: run with nothing pending honest · handle: run with holes refuses (does not dispatch) · amend merges prompt · drop clears pending · auto-tool: run/skip/name when pending, scan never stolen, bare run needs pending · catalog+tool sheet advertise script (not shelved) · capability guide + tool help tell the AI not to cmd the file · upload wired (console accept + handleScriptUpload + tool + detectScript) · upload HTTP: .txt preview not run + stashes pending.
  - **Mutations coming up:** G (YOURNAME not flagged) / H (skip steps ignored) / I (run auto-executes on upload) — each kills exactly its pins.
...
- **v0.27.2 — PHASE E item 2: vision auto-verify loop — ✅**:
  ...
  - `GHBot-0.27.2.jar` shipped (0.27.1 deleted per policy; GitHub Release mirrors).
  - **Phase E queue complete.** Optional owner follow-up: enable Gemini or a multimodal Ollama and set `build.verify-vision: true`.

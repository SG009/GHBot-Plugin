...
**Status: ALL PHASES COMPLETE (0–16) + 9b v2 Web Console + v0.21 Hardening + v0.21.39 pasted-spec direct-execute + v0.21.40 📎 upload & vision + v0.21.41 session eviction & thread safety + v0.21.42 mega builds (100k cap) & viewer action feed + v0.21.43 reload-reset & vision retry + v0.21.44 Stop button & Sponge v3 import fix · smoke **372/372 PASS** · jar `GHBot-0.21.44.jar`**
...
## GHBot v0.21.44 — current state & agent handoff brief (READ THIS FIRST)
...
2. Recompile + run the smoke suite (currently **372 checks**):
   ```bash
...
1. Drop `GHBot-0.21.44.jar` into `plugins/` → restart. Console: `/gh status`, `/gh device-info`.
...
└── tools/SmokeTest.java              (372 checks, all passing) · setup-build.sh (sandbox toolchain restore)
releases/GHBot-0.21.44.jar            (current ship; old jars removed once confirmed)
